...
import org.alexys.rest.excepciones.DatosNoEncontradosExcepcion;
...
public class ArticuloServicio {
	...
	public Articulo getArticulo(int id) {
		...
		if (respuesta == null) {
			System.out.println("iniciando excepcion");			
    ...
    public List<Articulo> getArticulosPorFecha(int year,int month){
    	...
    	for(Articulo articulo : listado) {
    		...
    		if(fecha.get(Calendar.YEAR) == year && fecha.get(Calendar.MONTH) == month - 1) {
    			resultado.add(articulo);
    		}
